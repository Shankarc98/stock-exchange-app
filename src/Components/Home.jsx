...
function Home(props){
  ...
  function handleClick(a, name, cp){
    ...
    companies.map((c) => {
     ...
     function handleTrade(trade, id, stocks){
      ...
      async function handleBuy(id, stocks){
        const response =  await fetch(`http://localhost:8080/trade/${player.id}/buy`, {
          method: "PUT",
        ...
        if(response.ok){
          ...
          let savedPlayer = await fetch(`http://localhost:8080/${player.id}/transaction`, {
            method: "PATCH", 
    ...
    async function handleSell(id, stocks){
       ...
       const response =  await fetch(`http://localhost:8080/trade/${player.id}/sell`, {
          method: "PUT",
